...
class Bob {
    constructor (opts = {}) {
        ...
        this.k8sCertApi = this.kc.makeApiClient(k8s.CertificatesV1beta1Api)
        ...
        if (opts.geoHash !== undefined ) {
            ...
            opts.bobNodeId = myenv.prefixGeohash(opts.geoHash, opts.bobNodeId)
        }

        dns.lookup(myenv.bootstrapHost, (err, result) => {
            var host = result
            if (err)
                host = myenv.bootstrapHost // assume it is IP or localhost
    ...
    cleanupK8s(token) {
        //exec(`${__dirname}/k8s_reset.sh`, (error, stdout, stderr) => {
        //    console.log(stdout)
        //    console.log('cleaned up k8s for next run')
        //}) // too slow - kubectl takes like 10s
    ...
    provisionUser(token) {
        ...
        status:
          conditions:
          - message: "Approved by AkriDEX"
            reason: "ApprovedByAkriDEX"
            type: "Approved"
            status: "True"
        `
        ...
        this.k8sCertApi.createCertificateSigningRequest(yamlCsrRequest).then(
            (response) => {
                ...
                return this.k8sCertApi.replaceCertificateSigningRequestStatus(token, yamlCsrApprove)
            },
        ).then(
            (response) => {
                console.log('updated CSR status');
