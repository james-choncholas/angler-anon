...
const akridex = require('akridex-discovery')
...
class AkriDEXOperator {
    constructor() {
        ...
        this.bob = new akridex.Bob({
            dhtPort: parseInt(portStart),
            webPort: parseInt(portStart) + 1,
            mpcPort: parseInt(portStart) + 2,
            geoHash: process.env.GEO_PREFIX ? process.env.GEO_PREFIX : undefined,
            onSuc: (token) => {
    ...
    async start() {
        // Create CRD
        const crdFile = `${__dirname}/MarketUnitCRDv1.yaml`
        ...
        this.watchReq = watch.watch(`/apis/${this.crdGroup}/${this.crdVersion}/${this.crdPlural}`,
            ...
            (phase, apiObj, watchObj) => {
                ...
                console.log(`unprefixed MarketUnit infoHash: ${infoHash.toString('hex')}`)
                infoHash = Buffer.concat([Buffer.from(loc), infoHash.slice(loc.length)]).toString('hex')
                console.log(`prefixed MarketUnit infoHash: ${infoHash}`)
