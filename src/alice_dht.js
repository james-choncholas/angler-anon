...
class Alice {
    constructor (opts = {}) {
        ...
        if (opts.geoHash !== undefined ) {
            ...
            opts.aliceNodeId = myenv.prefixGeohash(opts.geoHash, opts.aliceNodeId)
        }

        dns.lookup(myenv.bootstrapHost, (err, result) => {
            var host = result
            if (err)
                host = myenv.bootstrapHost // assume it is IP, localhost, private DNS
    ...
    readPodsUntilFail(myk8sApi, name, retries=40) {
        return new Promise((resolve, reject) => {
            return myk8sApi.listNamespacedPod(name).then(
                (response) => {
                    setTimeout(() => {
                        console.log(`read still works - re-reading - ${retries} retries left`);
                        if (retries < 1) return reject('max retries exceeded')
                        return this.readPodsUntilFail(myk8sApi, name, retries-1).then(resolve,reject)
                    }, 500)
                },
                (err) => {
                    return resolve('pod read failed successfully')
                },
            ).catch((reason) => {
        ...
        request.post({
            ...
            k8sApi.listNamespacedPod(token).then(
                (response) => {
                    ...
                    console.log('waiting for connection to die');
                    this.readPodsUntilFail(k8sApi, token).then(() => {
                        console.log('connection down - shutting down');
                        this.shutdown(this.onSuc)
                        return
                    }).catch(() => {
                        console.log('\n\nerror waiting for k8s access to die'.red)
                        this.shutdown(this.onErr)
                        return
                    })
                },
...
if (require.main === module) {
    ...
    if (cmdargs.length < 3 || cmdargs.length > 6) {
        console.log("Usage: " + process.argv[0] + process.argv[1] + " <port> <geohash> <lookup infohash> [nodeid] [msdelay for logger] [expected number of peers]")
        process.exit(1)
    }
